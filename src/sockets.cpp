...
{
	if (daemon_mode) {				//进程保护模式参数判断
		return;
	}
	char vbuffer[255];				//缓冲区
	va_list args;					//va_list类型指针
	va_start(args, message);		//获取可变参数列表的第一个参数,并存入args
	vsnprintf(vbuffer, ARRAY_SIZE(vbuffer), message, args);  //将可变参数传入数组
	va_end(args);					//清理指针
	time_t now;
	time(&now);						//获取当前系统时间
	char* date = ctime(&now);		//将时间转化为字符串
	date[strlen(date) - 1] = '\0';	
	unsigned long int self = (unsigned long
		int)pthread_self();			//获得当前线程自身ID
	if (errno != 0) {//发生错误
		...
		fprintf(log_file, "[%s][%lu] Info: %s\n", date, self,vbuffer);//输出正常
	}
	fflush(log_file);//刷新
}
int readn(int fd, void* buf, int n) //从fd中读取信息
{
	int nread, left = n;
	while (left > 0) {
...
int writen(int fd, void* buf, int n)//向fd中写入信息
{
	int nwrite, left = n;
	while (left > 0) {
...
int socks_invitation(int fd, int* version)
{
	...
	if (nread == 2 && init[0] != VERSION5 && init[0] !=
		VERSION4) {
		log_message("They send us %hhX %hhX", init[0],
			init[1]);
...
int socks5_auth_userpass(int fd)					//认证用户密码,若返回值为0则认证成功
{
	char answer[2] = { VERSION5, USERPASS };
	writen(fd, (void*)answer, ARRAY_SIZE(answer));	//向fd中写入版本信息与认证类型
	char resp;
	...
	if (strcmp(arg_username, username) == 0&& strcmp(arg_password, password) == 0) {//用户名密码匹配成功
		char answer[2] = { AUTH_VERSION, AUTH_OK };
		writen(fd, (void*)answer, ARRAY_SIZE(answer));//向fd中写入认证成功信息
		free(username);
		free(password);
		return 0;
	}
		...
		writen(fd, (void*)answer, ARRAY_SIZE(answer));//写入认证失败信息
		free(username);
		free(password);
		return 1;
	}
}
int socks5_auth_noauth(int fd)			//不认证
{
	char answer[2] = { VERSION5, NOAUTH };
	writen(fd, (void*)answer, ARRAY_SIZE(answer));
	return 0;
}
void socks5_auth_notsupported(int fd)	//没有方式认证
{
	char answer[2] = { VERSION5, NOMETHOD };
	writen(fd, (void*)answer, ARRAY_SIZE(answer));//向fd中写入未收到认证方式
}
/*sock认证*/
void socks5_auth(int fd, int methods_count)
{
	...
	for (int i = 0; i < num; i++) {//从fd中读取认证类型
		char type;
		readn(fd, (void*)&type, 1);
		log_message("Method AUTH %hhX", type);
		if (type == auth_type) {		//认证成功
			supported = 1;
		}
	}
	if (supported == 0) {				//未收到认证方式
		socks5_auth_notsupported(fd);
		app_thread_exit(1, fd);
	}
	int ret = 0;
	switch (auth_type) {		//分类认证方式
	case NOAUTH:
		ret = socks5_auth_noauth(fd);		//不认证
		break;
	case USERPASS:							
		ret = socks5_auth_userpass(fd);		//认证用户密码
		break;
	}
	if (ret == 0) {
		return;
...
char* socks_ip_read(int fd)
{
	...
	log_message("IP %hhu.%hhu.%hhu.%hhu", ip[0], ip[1], ip[2],
		ip[3]);
	return ip;
}
...
{
	return (ip[0] == 0 && ip[1] == 0 && ip[2] == 0 && ip[3] !=
		0);
}
...
{
	char resp[8] = { 0x00, (char)status, 0x00, 0x00, 0x00,
	0x00, 0x00, 0x00 };
...
void* app_thread_process(void* fd)
{
	...
	switch (version) {//选择版本
	case VERSION5: {
		socks5_auth(net_fd, methods);			//认证socks5的用户密码
		int command = socks5_command(net_fd);	//获得命令
		if (command == IP) {					//对IP命令进行处理
			char* ip = socks_ip_read(net_fd);	//获得IP
			unsigned short int p = socks_read_port(net_fd);//获得端口
			inet_fd = app_connect(IP, (void*)ip, ntohs(p));//建立连接
			if (inet_fd == -1) {
				app_thread_exit(1, net_fd);
			}
			socks5_ip_send_response(net_fd, ip, p);//向fd中写入响应、ip和端口信息
			free(ip);
			break;
		}
		else if (command == DOMAIN) {			//对域命令进行处理
			unsigned char size;
			char* address = socks5_domain_read(net_fd,&size);	//获得地址
			unsigned short int p = socks_read_port(net_fd);		//获得端口
			inet_fd = app_connect(DOMAIN, (void*)address,ntohs(p));	//建立连接	
			if (inet_fd == -1) {
				app_thread_exit(1, net_fd);
			}
			socks5_domain_send_response(net_fd, address,size, p);		//向net_fd中写入响应,地址,地址大小和端口信息
			free(address);
			break;
		}
		else {
	...
	case VERSION4: {
		if (methods == 1) {
			char ident[255];									
			unsigned short int p = socks_read_port(net_fd);		//获得端口号
			char* ip = socks_ip_read(net_fd);
			socks4_read_nstring(net_fd, ident,sizeof(ident));	//从net_fd中获取信息并存入ident
			if (socks4_is_4a(ip)) {				//域命令处理
				char domain[255];
			...
			if (inet_fd != -1) {
				socks4_send_response(net_fd, 0x5a);//向net_fd写入状态，响应信息
			}
			else {
				socks4_send_response(net_fd, 0x5b);//向net_fd写入状态，响应信息
				free(ip);
				app_thread_exit(1, net_fd);
			}
			free(ip);
	...
	app_socket_pipe(inet_fd, net_fd);//inet_fd与net_fd建立通信通道
	close(inet_fd);
...
int app_loop()
{
	int sock_fd, net_fd;
	int optval = 1;
	struct sockaddr_in local, remote;		//套接字结构体
	socklen_t remotelen;					//remote长度
	if ((sock_fd = socket(AF_INET, SOCK_STREAM, 0)) < 0) {			//创建套接字
		log_message("socket()");
		exit(1);
	}
	if (setsockopt(sock_fd, SOL_SOCKET, SO_REUSEADDR, (char*)&optval,sizeof(optval)) < 0) {//设置与某个套接字关联的选项，成功返回0，失败返回-1 
		log_message("setsockopt()");
		exit(1);
	}
	memset(&local, 0, sizeof(local));
	local.sin_family = AF_INET;
	local.sin_addr.s_addr = htonl(INADDR_ANY);//该服务器上所有的网卡,外部的client ask 不管从哪个server的地址近来都可以连接到1080端口.
...
int main(int argc, char* argv[])
{
	...
	while ((ret = getopt(argc, argv, "n:u:p:l:a:hd")) != -1) {//获取选项
		switch (ret) {
		case 'd': {								//进入进程保护模式
			daemon_mode = 1;
			daemonize();
